          ...
          <article key={`${route.from}-${route.to}`} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="font-bold text-slate-900">{route.note}</p>
                <p className="mt-1 text-sm text-slate-500">{route.modeLabel}</p>
                <p className="mt-2 inline-flex rounded-full bg-white px-3 py-1 text-sm font-bold text-blue-700">
                  {route.duration}
                </p>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="mt-3 block text-sm font-semibold text-blue-600 underline underline-offset-4"
                >
                  開啟 Google Maps 路線
                </a>
              </div>
